...
> Development builds prefer a narrow Accessibility operation that replaces only the captured selection. They confirm the inserted UTF-16 range when the control supports it, or the exact new caret when full readback is unavailable. Controls such as some Notes versions can use a one-shot, target-PID Unicode event after a second exact focus check. That debug-only fallback cannot atomically bind an event to one Accessibility element, so it never retries and reports an indeterminate result unless the original field confirms the expected caret and contents. Unsupported controls keep the transcript available for an explicit copy.
...
- Microphone capture, selectable input devices, a live level meter, verified local model downloads, cancellation, and local `whisper.cpp` batch transcription are implemented. A buffer overrun aborts before transcription so discontinuous audio can never become inserted text. Cleanup is off by default. Its conservative local option removes a reviewed set of pause-marked hesitation sounds in English, Spanish, French, German, Hindi, Italian, Russian, Japanese, and Chinese only when punctuation can be repaired safely; bare words, identifiers, quoted uses, explicit references, unknown languages, and unreviewed languages are left alone. Voice activity detection and translation are not connected yet.
