...
- Automatic insertion is not enabled in the normal build. A guarded InputMethodKit prototype can arm one exact input session, consume it once, and read the inserted text back before reporting success. It remains behind a Cargo feature until Spick and its helper mutually verify their release signatures and the compatibility matrix is measured. A valid transcript otherwise stays in memory on Today until the next recording, where the user can copy it explicitly.
