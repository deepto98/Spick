...
- The cleanup choice is saved natively and captured per session. As-transcribed output is the safe default; the opt-in local cleaner removes a reviewed, language-tagged set of pause-marked hesitation sounds across nine languages when punctuation repair is unambiguous, while preserving bare identifiers, quoted uses, explicit references, and unreviewed languages.
