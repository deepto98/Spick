...
Normal macOS builds now reach step 9 through two guarded paths. Compatible native controls receive an element-addressed Accessibility selection replacement, followed by exact content readback or exact-caret confirmation when parameterized readback is unavailable. Web, Electron, and other custom controls can use a guarded paste transaction only when the transcript contains no carriage return or line feed. Multiline text is refused before clipboard access, so this fallback is not a terminal insertion path; direct Accessibility and optional InputMethodKit insertion are unaffected. Spick retains up to 64 MiB of the general pasteboard snapshot in memory, although AppKit materializes lazy representations before that cap can be applied. It then revalidates the exact original target immediately before one PID-scoped `Cmd-V` and attempts to restore the snapshot only while its change-count and ownership marker still match. If ownership is already lost, it leaves the current clipboard alone. The public pasteboard API has no atomic compare-and-swap, so a concurrent clipboard change can still land between the final check and replacement. A selectionless custom field cannot prove that its original caret or selection survived, so its dispatch remains indeterminate; indeterminate delivery is never retried. macOS can deny or prompt for clipboard access under its privacy policy.
...
Settings schema v6 records the explicit cleanup choice together with language routing, microphone selection, compact HUD dock/visibility, privacy choices, and the shortcut. The v1 migration turns cleanup off because older builds wrote `readable-v1` as a default before the cleaner was connected; that stored value cannot prove consent. The v5 migration moved the old untouched bottom-center preset to the former bottom-right location; the v6 migration retires expanded/freeform placement and snaps the widget to the nearest left, right, or bottom dock. Other migrations preserve explicit choices, add safe defaults for newly persisted controls, and keep the original file as a backup.
